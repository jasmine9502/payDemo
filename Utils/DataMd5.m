                          ...
                          tradeType:(NSString *)tradeType
{
        if (self = [super init]) {
            ...
            [self.dic setValue:_wechatAppId forKey:@"appid"];
            [self.dic setValue:_wechatMCHId forKey:@"mch_id"];
            [self.dic setValue:_tradeNo     forKey:@"nonce_str"];
            [self.dic setValue:_payTitle    forKey:@"body"];
            [self.dic setValue:_orderNo     forKey:@"out_trade_no"];
            [self.dic setValue:_totalFee    forKey:@"total_fee"];
            [self.dic setValue:_deviceIp    forKey:@"spbill_create_ip"];
            [self.dic setValue:_notifyUrl   forKey:@"notify_url" ];
            [self.dic setValue:_tradeType   forKey:@"trade_type"];
...
-(void)createMd5Sign:(NSMutableDictionary*)dict
{
    ...
    //添加商户密钥key字段
    [contentString appendFormat:@"key=%@",WXSecret];
